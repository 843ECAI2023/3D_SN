...
from tensorflow.keras.layers import Input, Conv2D, GlobalAveragePooling2D,GlobalMaxPooling2D, Reshape, Dense, multiply,MaxPooling2D, AveragePooling2D,BatchNormalization, Activation

class Adapted_Feature_Extraction(object):
    ...
    def __call__(self, inputs):
        ...
        for i in range(a.shape[1]):
            fc_result = self.tensor_split(a)[i]
            for j in range(2):
              fc_result = self.generate_fc_layers(inputs)[j](fc_result)
            output_tensors_a.append(fc_result)
        
        output_tensors_b = []
        for i in range(b.shape[1]):
            fc_result = self.tensor_split(b)[i]
            for j in range(2):
              fc_result = self.generate_fc_layers(inputs)[j](fc_result)
            output_tensors_b.append(fc_result)
            
            
        multiplied_tensors_a = self.elementwise_multiply(self.tensor_split(a), output_tensors_a,a.shape[1])    
        added_tensor_a = tf.math.add_n(multiplied_tensors_a)    
        softmax_tensors_a = []
        for i in range(a.shape[1]):
            softmax_result_a = tf.math.divide(multiplied_tensors_a[i], added_tensor_a)
            softmax_tensors_a.append(softmax_result_a)    
        merged_tensor_a = tf.concat(softmax_tensors_a, axis=1)            

        multiplied_tensors_b = self.elementwise_multiply(self.tensor_split(b), output_tensors_b,b.shape[1])    
        added_tensor_b = tf.math.add_n(multiplied_tensors_b)    
        softmax_tensors_b = []
        for i in range(b.shape[1]):
            softmax_result_b = tf.math.divide(multiplied_tensors_b[i], added_tensor_b)
            softmax_tensors_b.append(softmax_result_b)    
        merged_tensor_b = tf.concat(softmax_tensors_b, axis=1)          
        
        return merged_tensor_a, merged_tensor_b

    def generate_fc_layers(self,inputs):

        num_outputs = inputs[0].shape[-1]
        fc_layers = []
        for j in range(2):
            if j == 1:                
                fc_layer = tf.keras.layers.Dense(units=num_outputs, activation=tf.nn.sigmoid)
            else:                
                fc_layer = tf.keras.layers.Dense(units=num_outputs, activation=tf.nn.relu)
            fc_layers.append(fc_layer)
            
        return fc_layers
    
    def tensor_split(self,input):
        return tf.split(input, num_or_size_splits=input.shape[1], axis=1)
    
    def elementwise_multiply(self,original_tensors, output_tensors ,split_num):
        
        assert len(original_tensors) == split_num and len(output_tensors) == split_num
    
        multiplied_tensors = []
        for i in range(split_num):
            multiplied_tensor = tf.math.multiply(original_tensors[i], output_tensors[i])
            multiplied_tensors.append(multiplied_tensor)
    
        return multiplied_tensors    
        
        
class Spatial_Attention(object):
    
    def __call__(self, inputs,reduction=4, activation=None):
        
        return self.attention(inputs[0],inputs[1],reduction=reduction, activation=None),self.attention(inputs[1],inputs[0],reduction=reduction, activation=None)
        
        
    def attention(self,x1,x2,reduction=4, activation=None):

        batch_size, height, width, channels = x1.shape.as_list()

        f = layers.Conv2D(channels // reduction, 1, activation=activation)(x1)
        g = layers.Conv2D(channels // reduction, 1, activation=activation)(x2)
        h = layers.Conv2D(channels, 1, activation=activation)(x2)
...
class Feature_Attention(object):
    
    def __call__(self, inputs,type='FA_1',reduction=4, activation=None):
        
        if type=='FA_1':
            return self.fa_1(inputs[0],reduction=reduction), self.fa_1(inputs[1],reduction=reduction)
        if type=='FA_2':
            return self.fa_2(inputs[0],reduction=reduction), self.fa_2(inputs[1],reduction=reduction)        
        if type=='FA_3':
            return self.fa_3(inputs[0],reduction=reduction), self.fa_3(inputs[1],reduction=reduction)
        
    def fa_1(self,input,reduction=4):
        batch_size, height, width, channels = input.shape.as_list()
        se_shape = (1, 1, channels)
        output = layers.GlobalAveragePooling2D()(input)
        output = layers.Reshape(se_shape)(output)
        output = layers.Dense(channels // (reduction//2), activation='relu', kernel_initializer='he_normal')(output)
        output = layers.Dense(channels//reduction, activation='sigmoid', kernel_initializer='he_normal')(output)
        
        return output 
    
    def fa_2(self,input,reduction=4):
        batch_size, height, width, channels = input.shape.as_list()
        ...
        shared_dense = layers.Dense(units=channels, activation="relu")(concat)
        sigmoid_dense = layers.Dense(units=channels, activation="sigmoid")(shared_dense)
        output = layers.Reshape((1, 1, channels))(sigmoid_dense)
    
        return output      


    def fa_3(self, input,reduction=4,activation=None):
        
        batch_size, height, width, channels = input.shape.as_list()
        
        x_h = layers.MaxPooling2D(pool_size=(1, width), strides=(1, 1), padding='valid')(input)
        x_w = layers.MaxPooling2D(pool_size=(height, 1), strides=(1, 1), padding='valid')(input)
        x_h = layers.Reshape((1,height,x_h.shape[3]))(x_h)
        x_hw = layers.concatenate([x_w, x_h], axis=2)  
        x_hw = layers.BatchNormalization(layers.Activation('sigmoid')(x_hw))        
        
        a = x_hw[:, :height, :, :]
        b = x_hw[:, height:, :, :]
        
        
        c = layers.Reshape((batch_size, 1, width, channels))(b)
        
        a = layers.Conv2D(channels // reduction, 1, activation=activation)(a)
        c = layers.Conv2D(channels // reduction, 1, activation=activation)(c)
        
        
        output = layers.Dot(axes=[3, 3])([a, c])  
...
class RFM(object):
    ...
    def __call__(self,inputs,input_filters=512,output_filters=128,stride=1,map_reduce=5):
        return self.rfm(inputs[0], input_filters, output_filters),self.rfm(inputs[1], input_filters, output_filters)
        
    def rfm(self,input,input_filters,output_filters,stride=1,map_reduce=5):
        
        input_filters_div = input_filters//map_reduce
    
        branch_0 = self.conv2d_bn(input,input_filters_div,1,1,stride=stride)
        branch_0 = self.conv2d_bn(branch_0,input_filters_div,3,3,relu=True)
    
        branch_1 = self.conv2d_bn(input,input_filters_div,1,1)
        branch_1 = self.conv2d_bn(branch_1,input_filters_div,1,1,stride=stride)
        
        branch_2 = self.conv2d_bn(input,input_filters_div,1,1)
        branch_2 = self.conv2d_bn(branch_2,input_filters_div,3,3,stride=stride)
    
        branch_3 = self.conv2d_bn(input,input_filters_div,1,1)
        branch_3 = self.conv2d_bn(branch_3,input_filters_div,5,5,stride=stride)
    
        out = layers.Concatenate([branch_0,branch_1,branch_2,branch_3],axis=-1)
