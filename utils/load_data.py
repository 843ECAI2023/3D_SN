...
def load_data(file, data_size=None,maxlen=64):
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    path = os.path.join(os.path.dirname(__file__), '../' + file)
    df = pd.read_csv(path)
    p = df['sentence1'].values[0:data_size].tolist()
    h = df['sentence2'].values[0:data_size].tolist()
    label = df['label'].values[0:data_size].tolist()
    
    encoded_p = tokenizer.batch_encode_plus(p,
    ...
    encoded_h = tokenizer.batch_encode_plus(h,
      add_special_tokens=True,
      truncation=True,
      max_length=maxlen,
      padding='max_length',
    ...
    encoded_p, encoded_h, label = shuffle(encoded_p, encoded_h, label)
